...
// The "loved it" mark on activity cards: a solid heart, colour set via
// currentColor so it matches the star fill in the row. The viewBox is
// cropped tight to the heart's own bounds so the glyph fills its box the
// same way a star does — that keeps it the same visual size as the stars
// and lets it sit right up against them instead of floating apart on its
// own internal padding.
export function iconHeartFilled() { return `<svg viewBox="3 4 18 18" fill="currentColor"><path d="M12 20.3 4.3 12.6A4.7 4.7 0 0 1 11 6l1 1 1-1a4.7 4.7 0 0 1 6.7 6.6z"/></svg>`; }
...
// "New message" — a pencil writing on paper, the standard compose glyph.
export function iconCompose() { return `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4H6a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-6"/><path d="M18.5 2.5a2.12 2.12 0 0 1 3 3L12 15l-4 1 1-4z"/></svg>`; }
...
// A STROKE gear, six teeth and a hollow centre. The old one was a
// solid Material cog — the only filled glyph in an app whose icons
// are all outlines — and at the 19px it actually renders at, solid
// lobes around a small hole read as a sun rather than a gear. The
// outline is generated from one formula (outer radius 10, root 7.4,
// six teeth) so the teeth are even, which is the other half of why
// the old shape was hard to read small.
// G08 from the "54 pieces" review — 8 teeth, heavy corner rounding,
// filled rather than stroked (softer than the stroke gear this
// replaces), the hole cut with evenodd instead of a second stroked
// circle. The path is the exact output of gears.py's s08(), computed
// once and pasted in rather than generated at runtime — this app has
// no build step to run that math in.
export function iconSettings() { return `<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M 19.01 9.11 Q 19.19 9.54 19.65 9.57 L 20.41 9.63 Q 21.53 9.71 21.53 10.84 L 21.53 13.16 Q 21.53 14.29 20.41 14.37 L 19.65 14.43 Q 19.19 14.46 19.01 14.89 L 19.00 14.92 Q 18.83 15.34 19.12 15.69 L 19.62 16.27 Q 20.36 17.12 19.56 17.92 L 17.92 19.56 Q 17.12 20.36 16.27 19.62 L 15.69 19.12 Q 15.34 18.83 14.92 19.00 L 14.89 19.01 Q 14.46 19.19 14.43 19.65 L 14.37 20.41 Q 14.29 21.53 13.16 21.53 L 10.84 21.53 Q 9.71 21.53 9.63 20.41 L 9.57 19.65 Q 9.54 19.19 9.11 19.01 L 9.08 19.00 Q 8.66 18.83 8.31 19.12 L 7.73 19.62 Q 6.88 20.36 6.08 19.56 L 4.44 17.92 Q 3.64 17.12 4.38 16.27 L 4.88 15.69 Q 5.17 15.34 5.00 14.92 L 4.99 14.89 Q 4.81 14.46 4.35 14.43 L 3.59 14.37 Q 2.47 14.29 2.47 13.16 L 2.47 10.84 Q 2.47 9.71 3.59 9.63 L 4.35 9.57 Q 4.81 9.54 4.99 9.11 L 5.00 9.08 Q 5.17 8.66 4.88 8.31 L 4.38 7.73 Q 3.64 6.88 4.44 6.08 L 6.08 4.44 Q 6.88 3.64 7.73 4.38 L 8.31 4.88 Q 8.66 5.17 9.08 5.00 L 9.11 4.99 Q 9.54 4.81 9.57 4.35 L 9.63 3.59 Q 9.71 2.47 10.84 2.47 L 13.16 2.47 Q 14.29 2.47 14.37 3.59 L 14.43 4.35 Q 14.46 4.81 14.89 4.99 L 14.92 5.00 Q 15.34 5.17 15.69 4.88 L 16.27 4.38 Q 17.12 3.64 17.92 4.44 L 19.56 6.08 Q 20.36 6.88 19.62 7.73 L 19.12 8.31 Q 18.83 8.66 19.00 9.08 L 19.01 9.11 Z M 8.80 12.00 A 3.20 3.20 0 1 0 15.20 12.00 A 3.20 3.20 0 1 0 8.80 12.00 Z" fill="currentColor" fill-rule="evenodd"/></svg>`; }
// S05 from the same review — a curved "send" arrow, filled, the one
// the profile top bar's left button uses instead of the 3-dot menu.
// A dedicated icon rather than repointing iconDotsMenu(), which
// Messages still uses unchanged.
export function iconCurvedShare() { return `<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M13 4.5v4.2C7.5 9.3 4.3 12.8 3.5 18.5c2.3-3.2 5.3-4.8 9.5-4.9v4.3l7.5-6.7z" fill="currentColor" stroke="none"/></svg>`; }
export function iconBell() { return `<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M18 9a6 6 0 1 0-12 0c0 5.2-1.6 6.6-2 7 .6.4 1.4.5 2 .5h12c.6 0 1.4-.1 2-.5-.4-.4-2-1.8-2-7z" stroke="currentColor" stroke-width="1.8" stroke-linejoin="round"/><path d="M10.3 20a2 2 0 0 0 3.4 0" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>`; }
...
// The "wrote a review" mark on activity cards: three short text lines,
// like a few lines of writing. Stroke-only, so it colours via currentColor
